...
/**
 * Stores a dictionary: a map of words to definitions. This class behaves like a
 * regular in-memory map, with an extra {@link #close()} method.
 * 
 * 
 *
 */
public interface PersistentDictionary {
    ...
    /**
     * Open and Load the dictionary. Accessing the dictionary is only allowed after
     * the first opening. Calling this method will undo any changes since the last
     * {@link #close()}.
     * 
     * @throws IOException
     */
    ...
    /**
     * Flush the contents of the dictionary to disk and close any open files. After
     * a close, the contents are guaranteed to persist across program executions.
     ...
     * mapping from a key k to a value v such that (key==null ? k==null :
     ...
     * specified value. (A map m is said to contain a mapping for a key k if and
     ...
     * if this map contains a mapping from key k to value v such that (key==null ?
     * k==null : key.equals(k)), that mapping is removed. (The map can contain at
     * most one such mapping.) Returns the value to which this map previously
